...
# Clean, simple blueprint matching user reference
SIMPLE_STRUCTURE = [
    {
...
async def on_ready():
    ...
    print(f"🎨 Restructuring {guild.name} into simple, clean layout...")

    # Delete all existing channels to make a clean, pristine slate
    for chan in list(guild.channels):
        try:
            await chan.delete(reason="Applying clean simple server layout")
            print(f"Deleted old channel/cat: {chan.name}")
            await asyncio.sleep(0.3)
        except Exception as e:
            print(f"Error deleting {chan.name}: {e}")

    # Build the simple structure
    for section in SIMPLE_STRUCTURE:
        ...
        for c_info in section["channels"]:
            ...
            if c_type == "text":
                ...
                print(f"  + Text Channel: #{c_name}")
            elif c_type == "voice":
                c = await guild.create_voice_channel(name=c_name, category=category, user_limit=limit)
                print(f"  + Voice Channel: 🔊 {c_name} (Limit: {limit})")
            await asyncio.sleep(0.4)

    # Now deploy clean embeds into verify, rules, self-roles, and ticket channels
    ...
    if v_chan:
        ...
        print("✅ Deployed Verification Gate!")
    ...
    if t_chan:
        ...
        print("✅ Deployed Ticket Desk!")

    print("\n🎉 SIMPLE LAYOUT COMPLETE & MATCHES YOUR EXACT REFERENCE!")
    await client.close()
