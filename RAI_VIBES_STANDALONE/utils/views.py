...
class AddSongModal(Modal, title="Add Song to Queue"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            from cogs.music import Song
            song_obj = await Song.create_source(query, interaction.user, self.music_cog.bot.loop if self.music_cog else None)
            if song_obj:
...
class SearchSongModal(Modal, title="Search Music"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        loop = self.music_cog.bot.loop if self.music_cog else asyncio.get_event_loop()
        try:
...
class SearchSelectView(View):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.defer(ephemeral=True)
        from cogs.music import Song
        song_obj = await Song.create_source(entry.get("webpage_url") or entry.get("url"), interaction.user, self.music_cog.bot.loop if self.music_cog else None)
        if song_obj:
            is_queued = self.player.enqueue_track(song_obj)
...
class RythmControllerView(View):
    ...
    async def prev_btn(self, interaction: discord.Interaction, btn: Button):
        ...
        if played_sec > 3.0:
            ...
            from cogs.music import Song
            song_obj = await Song.create_source(prev_song.webpage_url or prev_song.title, interaction.user, self.music_cog.bot.loop if self.music_cog else None)
            if song_obj:
    ...
    async def queue_btn(self, interaction: discord.Interaction, btn: Button):
        player = self.player
        embed = player.build_queue_embed(page=0)
        view = QueuePaginationView(player, self.music_cog)
        await interaction.response.send_message(embed=embed, view=view, ephemeral=True)
