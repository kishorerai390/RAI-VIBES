...
def create_security_bot(use_members: bool = True, use_message_content: bool = True) -> commands.Bot:
    ...
    async def on_ready():
        ...
        # Update Sentinel bot banner to 3D animated GIF
        banner_path = os.path.join("assets", "rai_sentinel_3d_banner.gif")
        ...
        if os.path.exists(banner_path):
            try:
                if not bot.user.banner or not str(bot.user.banner).startswith("a_"):
                    ...
                    logger.info("✨ [RAI SENTINEL] Successfully applied 3D animated GIF profile banner!")
            except Exception as e:
