...
class Welcome(commands.Cog):
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        # De-duplicate: ignore if already welcomed in the last 60 seconds
        if not hasattr(self, "_recent_welcomes"):
            self._recent_welcomes = {}
        import time
        now = time.time()
        if member.id in self._recent_welcomes and (now - self._recent_welcomes[member.id]) < 60:
            return
        self._recent_welcomes[member.id] = now
